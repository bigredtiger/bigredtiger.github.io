...
var BRT = {
	LAYOUT: {
		...
		*/
		setMenu: function() {	
			var myMenu = '<ul>', myTitle = '', arrayMenu = DATA.menu, loc = document.location.toString(), cName;
	...
	EVENT: {
		...
		setGamesMenu: function() {
			var oEvt = null;
			document.getElementById('gamesNav').onclick = function(e) {
				try {/*
					var evt = window.event ? window.event.target : e.target;
					if (evt.id.indexOf('g_') + 1) {
						if (oEvt !== null) {
							oEvt.className = '';
						}
						oEvt = evt;
						evt.className = 'gSelected';
					}*/
				} catch (e) {
				}
			}
